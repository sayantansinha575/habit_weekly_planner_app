...
export const storage = {
  ...
  fetchTasks: async (userId: string) => {
    ...
    try {
      ...
      console.warn('Backend fetch failed Current date Task, loading from local storage', e);
      const jsonValue = await AsyncStorage.getItem(TASKS_KEY);
      return jsonValue != null ? JSON.parse(jsonValue) : [];
    }
  },
  ...
  updateTask: async (taskId: string, title: string, scheduledDate: Date, scheduledTime?: string, isNotificationEnabled: boolean = true) => {
    try {
      // 1. API Call
      const updatedTask = await api.updateTask(taskId, title, scheduledDate.toISOString(), scheduledTime, isNotificationEnabled);
  ...
  getUserStats: async (userId: string) => {
    try {
        ...
        return jsonValue != null ? JSON.parse(jsonValue) : { dailyStreak: 0, weeklyStreak: 0, completionRate: 0, bestDay: 'N/A' };
    }
  },
  ...
  applyTemplate: async (userId: string, templateId: string) => {
    try {
        ...
        // After applying, we should probably clear tasks cache or re-fetch
        return true;
    } catch (e) {
